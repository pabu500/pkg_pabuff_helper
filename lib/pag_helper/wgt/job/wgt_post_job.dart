...
class _WgtPagPostJobState extends State<WgtPagPostJob> {
  Future<dynamic> _postJob() async {
    try {
      ...
      jobRequest['job_task_type'] =
          getPagJobTaskTypeName(PagJobTaskType.itemHistory);
      // jobRequest['job_type'] = 'item-history';
      String projectLabel =
          widget.loggedInUser.selectedScope.projectProfile?.label ?? '';
      jobRequest['job_type'] = jobRequest['job_task_type'] +
          (projectLabel.isNotEmpty ? '-$projectLabel' : '');
