...
class _WgtItemGroupItemAssignmentState
  ...
  Future<void> _doAutoPopulate() async {
    ...
    try {
      ...
      for (Map<String, dynamic> itemInfo in _itemGroupScopeMatchingItemList!) {
        ...
        if (itemInfo['item_item_group_id'] != null) {
          itemInfo['assigned'] = true;
        }
        itemInfo['assigned_to_this_item_group'] = false;
        if (itemInfo['item_item_group_id'] == widget.strItemGroupIndex) {
          itemInfo['assigned_to_this_item_group'] = true;
        }
      }
      ...
      // find the item that is assigned to this item group,
      // and move it to the top
      for (Map<String, dynamic> itemInfo in _itemGroupScopeMatchingItemList!) {
        if (itemInfo['assigned_to_this_item_group'] != true) {
          continue;
        }
  ...
  Widget getScopeMismatchItemList(double listHeight) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          Text(
              'The following list contains the item(s) with mismatched scope to the item group. Please resolve the scope mismatch before proceeding with assignment operation.',
              style: TextStyle(
  ...
  Widget getScopeItemList() {
    ...
    return ListView.builder(
      key: UniqueKey(), // Add a key to force rebuild when the list changess
      // shrinkWrap: true,
      // itemExtent: 35,
      itemCount: itemWidgetList.length,
      itemBuilder: (context, index) {
  ...
  Widget getItemRow(Map<String, dynamic> itemInfo, int index) {
    ...
    String? itemScopeLocationId = itemInfo['item_scope_location_id'];
    String? itemScopeLocationName = itemInfo['item_scope_location_name'];
    String? itemScopeLocationLabel = itemInfo['item_scope_location_label'];

    String? itemScopeLocationGroupId = itemInfo['item_scope_location_group_id'];
    String? itemScopeLocationGroupName =
        itemInfo['item_scope_location_group_name'];
    String? itemScopeLocationGroupLabel =
        itemInfo['item_scope_location_group_label'];

    String? itemScopeBuildingId = itemInfo['item_scope_building_id'];
    String? itemScopeBuildingName = itemInfo['item_scope_building_name'];
    String? itemScopeBuildingLabel = itemInfo['item_scope_building_label'];

    String? itemScopeSiteId = itemInfo['item_scope_site_id'];
    String? itemScopeSiteName = itemInfo['item_scope_site_name'];
    String? itemScopeSiteLabel = itemInfo['item_scope_site_label'];

    String? itemScopeSiteGroupId = itemInfo['item_scope_site_group_id'];
    String? itemScopeSiteGroupName = itemInfo['item_scope_site_group_name'];
    String? itemScopeSiteGroupLabel = itemInfo['item_scope_site_group_label'];
          ...
          children: [
            ...
            Tooltip(
              ...
              child: InkWell(
                ...
                child: Icon(
                  _selectedItemIndexStr == itemInfo['id']
                      ? Symbols.expand_less
                      : Symbols.expand_more,
                  color: Theme.of(context).hintColor,
                ),
