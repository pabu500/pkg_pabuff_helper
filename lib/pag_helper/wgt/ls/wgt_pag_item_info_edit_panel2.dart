...
class WgtPagItemInfoEditPanel2 extends StatefulWidget {
  const WgtPagItemInfoEditPanel2({
    super.key,
  ...
  @override
  State<WgtPagItemInfoEditPanel2> createState() =>
      _WgtPagItemInfoEditPanel2State();
}

class _WgtPagItemInfoEditPanel2State extends State<WgtPagItemInfoEditPanel2> {
  ...
  Future<List<Map<String, dynamic>>> _updateProfile(
    ...
    try {
      ...
      Map<String, dynamic> queryMap = {
        ...
        'id_type': PagIdType.indexType.value,
        'item_id_key': 'id',
      ...
      final result = await ex(
        endpoint: PagUrlBase.eptDoOpKeyValUpdate,
        crudType: 'update',
        opStr: 'update key value',
      ...
      final opResultList = result;
      List<Map<String, dynamic>> listOped = [];
      for (Map<String, dynamic> item in opResultList) {
      ...
      result['error'] =
          getErrorText(e, defaultErrorText: 'Error updating field');
  ...
  Future<dynamic> _doDelete(String itemName) async {
    ...
    try {
      ...
      dev.log(e.toString());
      return {
        'error': getErrorText(e, defaultErrorText: 'Error deleting item')
      };
    } finally {
      setState(() {
        _isDeleting = false;
