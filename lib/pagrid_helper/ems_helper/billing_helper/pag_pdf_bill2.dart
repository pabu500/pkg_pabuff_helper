...
// Sub-Total (Non-Taxable) = (1) + (2) + (3)
  ...
  pw.Widget _getTypeStat(
    ...
    final excludeAutoUsageValue = singularUsageInfo['exclude_auto_usage'];
    final excludeAutoUsage = excludeAutoUsageValue == true ||
        excludeAutoUsageValue?.toString().toLowerCase() == 'true';
  ...
  pw.Widget _getManualUsage2(
    ...
    final normalizedMeterTypeTag = meterTypeTag.toLowerCase();
    final manualUsageInfo = singularUsageInfo['manual_usage_info'];
    dynamic typeUsage =
        singularUsageInfo['manual_usage_$normalizedMeterTypeTag'];
    if (typeUsage == null && manualUsageInfo is Map) {
      typeUsage = manualUsageInfo['manual_usage_$normalizedMeterTypeTag'];
    }
    if (typeUsage == null &&
        singularUsageInfo['meter_type']?.toString().toLowerCase() ==
            normalizedMeterTypeTag) {
      typeUsage = singularUsageInfo['manual_usage'];
    }

    final usage = typeUsage is num
        ? typeUsage.toDouble()
        : double.tryParse(typeUsage?.toString() ?? '');
    if (usage == null) {
      return pw.Container();
    }
